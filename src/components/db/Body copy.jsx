...
function Body() {
    ...
    const [modalMedia, setModalMedia] = useState(null); // Changed from modalImage to modalMedia
    const [modalMediaType, setModalMediaType] = useState(null); // 'image' or 'video'
    ...
    const handleFileChange = (event) => {
        ...
        // Set file size limit (10MB as in backend)
    ...
    const handleUpload = async (event) => {
        ...
        if (!postText && !selectedFile) {
            setUploadError("Please enter some text or upload an image/video.");
            return;
        }
        ...
        if (selectedFile) formData.append("file", selectedFile); // Changed from "image" to "file" to match backend
    ...
    // Function to handle opening media modal
        ...
        <div className="pt-10 flex min-h-screen bg-gray-100">
                ...
                <div className="text-center">
                    {userProfile?.profilePicture ? (
                        <img
                            ...
                            onClick={() => openMediaModal(`data:image/png;base64,${userProfile.profilePicture}`, 'image')}
                        />
                    ) : (
            ...
            <main className="flex-1 p-6">
                <form className="bg-white p-6 rounded-lg shadow-lg" onSubmit={handleUpload}>
                    ...
                    {/* Preview section for both images and videos */}
                    {preview && previewType === 'image' && (
                        <img 
                            src={preview} 
                            alt="Preview" 
                            className="mt-3 rounded-lg max-w-xs cursor-pointer" 
                            onClick={() => openMediaModal(preview, 'image')}
                        />
                    )}
                    ...
                    {preview && previewType === 'video' && (
                        <video 
                            src={preview} 
                            className="mt-3 rounded-lg max-w-xs cursor-pointer" 
                            onClick={() => openMediaModal(preview, 'video')}
                            controls
                        />
                    )}
                        ...
                        <div key={post._id} className="bg-white p-4 rounded-lg shadow-md">
                            ...
                            {post.image && (
                                <img 
                                    ...
                                    onClick={() => openMediaModal(`data:image/png;base64,${post.image}`, 'image')}
                                />
                            )}
                            ...
                            {post.video && (
                                <video 
                                    src={`data:${post.video.contentType};base64,${post.video.data}`}
                                    className="w-full max-h-96 object-contain mt-2 rounded-lg cursor-pointer"
                                    onClick={() => openMediaModal(`data:${post.video.contentType};base64,${post.video.data}`, 'video')}
                                    controls
                                    preload="metadata"
                                />
                            )}
            ...
            {/* Media Modal (for both images and videos) */}
            {modalMedia && (
                <div 
